export default {
  login: {
    title: 'Sign In',
    username: 'Username',
    password: 'Password',
    submit: 'Sign In',
    required: 'This field is required',
    failed: 'Login failed, please try again',
  },
  role: {
    title: 'Role Management',
    search: { roleName: 'Role Name', roleCode: 'Role Code', search: 'Search', reset: 'Reset' },
    table: {
      roleName: 'Role Name',
      roleCode: 'Role Code',
      description: 'Description',
      updateBy: 'Updated By',
      updateAt: 'Updated At',
      actions: 'Actions',
    },
    actions: { detail: 'Detail', edit: 'Edit', delete: 'Delete', create: 'Create Role' },
    detail: { title: 'Role Detail', close: 'Close' },
    form: {
      createTitle: 'Create Role',
      editTitle: 'Edit Role',
      roleName: 'Role Name',
      roleCode: 'Role Code',
      description: 'Description',
      inheritance: 'Role Inheritance',
      inheritanceHint: 'Select roles to inherit from the list below',
      searchPlaceholder: 'Search roles...',
      treeTitle: 'Inheritance Preview',
      treeEmpty: 'No roles selected. Check roles from the left list.',
      redundantWarning: 'Redundant: already inherited indirectly',
      removeRedundant: 'Remove',
      cancel: 'Cancel',
      submit: 'Submit',
    },
    delete: {
      title: 'Confirm Delete',
      message: 'Are you sure you want to delete this role? This action cannot be undone.',
      confirm: 'Delete',
      cancel: 'Cancel',
    },
    validation: {
      roleNameRequired: 'Please enter a role name',
      roleCodeRequired: 'Please select a role code',
    },
    error: {
      loadFailed: 'Failed to load role data',
      saveFailed: 'Failed to save',
      deleteFailed: 'Failed to delete',
      inheritanceSaveFailed: 'Failed to save inheritance',
    },
  },
  user: {
    title: 'User Management',
    search: { username: 'Username', status: 'Status', search: 'Search', reset: 'Reset' },
    table: {
      username: 'Username',
      status: 'Status',
      updateBy: 'Updated By',
      updateAt: 'Updated At',
      actions: 'Actions',
    },
    actions: { detail: 'Detail', edit: 'Edit', delete: 'Delete', create: 'Create User' },
    status: { enabled: 'Enabled', disabled: 'Disabled' },
    detail: { title: 'User Detail', close: 'Close' },
    form: {
      createTitle: 'Create User',
      editTitle: 'Edit User',
      username: 'Username',
      password: 'Password',
      status: 'Status',
      roles: 'Roles',
      cancel: 'Cancel',
      submit: 'Submit',
      passwordHint: 'Leave blank to keep current password',
    },
    delete: {
      title: 'Confirm Delete',
      message: 'Are you sure you want to delete this user? This action cannot be undone.',
      confirm: 'Delete',
      cancel: 'Cancel',
    },
    validation: {
      usernameRequired: 'Please enter a username',
      passwordRequired: 'Please enter a password',
      statusRequired: 'Please select a status',
    },
    error: {
      loadFailed: 'Failed to load user data',
      saveFailed: 'Failed to save',
      deleteFailed: 'Failed to delete',
    },
  },
}
