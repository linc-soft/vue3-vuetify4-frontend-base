export default {
  app: {
    logout: 'Sign Out',
    language: 'Language',
    changePassword: 'Change Password',
  },
  login: {
    title: 'Sign In',
    username: 'Username',
    password: 'Password',
    submit: 'Sign In',
    required: 'This field is required',
    failed: 'Login failed, please try again',
    forgotPassword: 'Forgot Password?',
  },
  forgotPassword: {
    title: 'Forgot Password',
    usernameOrEmail: 'Username or Email',
    submit: 'Send Reset Link',
    backToLogin: 'Back to Sign In',
    successMessage:
      'If an account with that username or email exists, a password reset link has been sent to the associated email address.',
    sendFailed: 'Failed to send reset link. Please try again.',
  },
  resetPassword: {
    title: 'Reset Password',
    newPassword: 'New Password',
    confirmNewPassword: 'Confirm New Password',
    submit: 'Reset Password',
    backToLogin: 'Back to Sign In',
    successMessage: 'Password has been reset successfully. Redirecting to sign in page...',
    tokenInvalidMessage: 'This reset link is invalid or has expired. Please request a new one.',
    passwordMismatch: 'Passwords do not match',
    required: 'This field is required',
    minLength: 'Password must be at least 8 characters',
  },
  changePassword: {
    title: 'Change Password',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    confirmNewPassword: 'Confirm New Password',
    submit: 'Change Password',
    cancel: 'Cancel',
    successMessage: 'Password has been changed successfully.',
    passwordMismatch: 'Passwords do not match',
    required: 'This field is required',
    minLength: 'Password must be at least 8 characters',
  },
  forceChangePassword: {
    title: 'Change Password',
    description: 'You are required to set a new password before continuing.',
    newPassword: 'New Password',
    confirmPassword: 'Confirm New Password',
    submit: 'Change Password',
    logout: 'Sign Out',
    required: 'This field is required',
    minLength: 'Password must be at least 8 characters',
    passwordMismatch: 'Passwords do not match',
    failed: 'Failed to change password. Please try again.',
  },
  role: {
    title: 'Role Management',
    search: { roleName: 'Role Name', roleCode: 'Role Code', search: 'Search', reset: 'Reset' },
    table: {
      roleName: 'Role Name',
      roleCode: 'Role Code',
      description: 'Description',
      updateBy: 'Updated By',
      updateAt: 'Updated At',
      actions: 'Actions',
    },
    actions: { detail: 'Detail', edit: 'Edit', delete: 'Delete', create: 'Create Role' },
    detail: {
      title: 'Role Detail',
      close: 'Close',
      parentRoles: 'Directly Inherited Roles',
      effectiveBaseRoles: 'Effective Base Roles',
      selfIsBaseRole: 'This role itself is a base role',
    },
    form: {
      createTitle: 'Create Role',
      editTitle: 'Edit Role',
      roleName: 'Role Name',
      roleCode: 'Role Code',
      description: 'Description',
      cancel: 'Cancel',
      submit: 'Submit',
      parentRoles: 'Inherited Roles',
      parentRolesHint: 'Multi-select; permissions of selected roles will be inherited',
      badgeBase: 'Base',
      badgeComposite: 'Composite',
      inheritsFrom: 'Inherits from',
      noParents: 'No inherited roles',
      previewTitle: 'Inheritance Preview',
      effectiveBaseRoles: 'Effective Base Roles',
      noBaseRoles: 'Selected parent roles contain no base roles',
    },
    delete: {
      title: 'Confirm Delete',
      message: 'Are you sure you want to delete this role? This action cannot be undone.',
      confirm: 'Delete',
      cancel: 'Cancel',
    },
    validation: {
      roleNameRequired: 'Please enter a role name',
    },
    error: {
      loadFailed: 'Failed to load role data',
      saveFailed: 'Failed to save',
      deleteFailed: 'Failed to delete',
    },
  },
  user: {
    title: 'User Management',
    search: { username: 'Username', status: 'Status', search: 'Search', reset: 'Reset' },
    table: {
      username: 'Username',
      email: 'Email',
      status: 'Status',
      updateBy: 'Updated By',
      updateAt: 'Updated At',
      actions: 'Actions',
    },
    actions: { detail: 'Detail', edit: 'Edit', delete: 'Delete', create: 'Create User' },
    report: {
      button: 'Generate Report',
      groupBy: 'Group By',
      groupByNone: 'No Grouping',
      groupByRole: 'By Role',
      groupByBaseRole: 'By Base Role',
      generate: 'Generate',
      cancel: 'Cancel',
      title: 'Generate User List Report',
      downloading: 'Generating report...',
    },
    status: { enabled: 'Enabled', disabled: 'Disabled', inactive: 'Inactive' },
    detail: { title: 'User Detail', close: 'Close', roles: 'Roles', noRoles: 'No roles assigned' },
    form: {
      createTitle: 'Create User',
      editTitle: 'Edit User',
      username: 'Username',
      password: 'Password',
      email: 'Email',
      status: 'Status',
      roles: 'Roles',
      rolesHint: 'Multi-select; permissions of selected roles will be granted to this user',
      compositeRole: 'Composite role',
      cancel: 'Cancel',
      submit: 'Submit',
      passwordHint: 'Leave blank to keep current password',
    },
    delete: {
      title: 'Confirm Delete',
      message: 'Are you sure you want to delete this user? This action cannot be undone.',
      confirm: 'Delete',
      cancel: 'Cancel',
    },
    validation: {
      usernameRequired: 'Please enter a username',
      passwordRequired: 'Please enter a password',
      statusRequired: 'Please select a status',
      emailInvalid: 'Please enter a valid email address',
      emailRequired: 'Email is required',
    },
    error: {
      loadFailed: 'Failed to load user data',
      saveFailed: 'Failed to save',
      deleteFailed: 'Failed to delete',
    },
  },
  log: {
    // Navigation
    nav: {
      title: 'Logs',
      access: 'Access Logs',
      error: 'Error Logs',
      operation: 'Operation Logs',
      trace: 'Trace Detail',
    },
    // Common
    common: {
      traceId: 'Trace ID',
      username: 'Username',
      createdAt: 'Time',
      search: 'Search',
      reset: 'Reset',
      export: 'Export',
      copy: 'Copy',
      collapse: 'Collapse',
      expand: 'Expand',
      noData: 'No data',
      loadMore: 'Load More',
      totalRecords: '{count} records in total',
    },
    // Access Log
    access: {
      title: 'Access Logs',
      traceId: 'Trace ID',
      username: 'Username',
      method: 'Method',
      path: 'API Path',
      statusCode: 'Status Code',
      duration: 'Duration',
      clientIp: 'Client IP',
      startTime: 'Start Time',
      endTime: 'End Time',
      timeRange: 'Time Range',
      queryString: 'Query String',
      requestBody: 'Request Body',
      responseBody: 'Response Body',
      userAgent: 'User Agent',
    },
    // Error Log
    error: {
      title: 'Error Logs',
      traceId: 'Trace ID',
      errorType: 'Error Type',
      keyword: 'Keyword',
      message: 'Message',
      stackTrace: 'Stack Trace',
      username: 'Username',
      startTime: 'Start Time',
      endTime: 'End Time',
      timeRange: 'Time Range',
      noError: 'No error in this request',
    },
    // Operation Log
    operation: {
      title: 'Operation Logs',
      traceId: 'Trace ID',
      operationType: 'Operation Type',
      module: 'Module',
      subModule: 'Sub Module',
      description: 'Description',
      duration: 'Duration',
      username: 'Username',
      startTime: 'Start Time',
      endTime: 'End Time',
      timeRange: 'Time Range',
      beforeData: 'Before',
      afterData: 'After',
      diff: 'Diff',
      showUnchanged: 'Show unchanged fields',
      requestMethod: 'Request Method',
      requestUrl: 'Request URL',
      clientIp: 'Client IP',
      types: {
        CREATE: 'Create',
        DELETE: 'Delete',
        EXPORT: 'Export',
        IMPORT: 'Import',
        LOGIN: 'Login',
        LOGOUT: 'Logout',
        OTHER: 'Other',
        QUERY: 'Query',
        UPDATE: 'Update',
      },
    },
    // Trace
    trace: {
      title: 'Trace Detail',
      totalDuration: 'Total Duration',
      operationCount: 'Operations',
      errorCount: 'Errors',
      noAccessLog: 'No access log recorded',
    },
  },
  // Leave management
  leave: {
    apply: {
      title: 'Apply for Leave',
      startDate: 'Start Date',
      endDate: 'End Date',
      leaveType: 'Leave Type',
      duration: 'Duration (days)',
      reason: 'Reason',
      attachments: 'Attachments',
      required: 'This field is required',
      invalidDuration: 'Duration must be a positive multiple of 0.5',
      submit: 'Submit',
      reset: 'Reset',
      cancel: 'Cancel',
      confirmTitle: 'Confirm Leave Application',
      confirmMessage: 'Are you sure you want to submit this leave application?',
      confirm: 'Confirm',
      success: 'Leave application submitted successfully',
      failed: 'Failed to submit leave application',
    },
    list: {
      year: 'Year',
      leaveType: 'Leave Type',
      status: 'Status',
      search: 'Search',
      reset: 'Reset',
      nickname: 'Employee',
      startDate: 'Start Date',
      endDate: 'End Date',
      duration: 'Duration',
      days: 'days',
      attachments: 'Attachments',
      actions: 'Actions',
      cancel: 'Cancel Leave',
      deleteTitle: 'Confirm Delete',
      deleteMessage: 'Are you sure you want to cancel this leave request?',
      confirm: 'Delete',
    },
    approval: {
      leaveType: 'Leave Type',
      search: 'Search',
      nickname: 'Employee',
      period: 'Period',
      duration: 'Duration',
      reason: 'Reason',
      attachments: 'Attachments',
      status: 'Status',
      actions: 'Actions',
      approve: 'Approve',
      reject: 'Reject',
      rejectTitle: 'Reject Leave Request',
      rejectReason: 'Reason for rejection',
      cancel: 'Cancel',
      confirm: 'Confirm',
    },
    info: {
      title: 'Annual Leave Information',
      effectiveDate: 'Effective Date',
      daysBefore: 'Days Before',
      daysAfter: 'Days After',
      remainDays: 'Remaining Days',
      leaveTypeInfo: 'Leave Type Usage',
    },
    init: {
      title: 'Initialize Annual Leave',
      user: 'Employee',
      year: 'Year',
      force: 'Force re-initialization',
      submit: 'Initialize',
      success: 'Annual leave initialized successfully',
      failed: 'Failed to initialize annual leave',
    },
    nav: {
      title: 'Leave',
      apply: 'Apply Leave',
      list: 'My Leaves',
      approval: 'Approve Leave',
    },
  },
  // Employee management
  employee: {
    title: 'Employee Management',
    search: {
      keyword: 'Search by name or email',
      status: 'Status',
      search: 'Search',
      reset: 'Reset',
    },
    table: {
      username: 'Username',
      nickname: 'Display Name',
      sex: 'Gender',
      hiredDate: 'Hire Date',
      status: 'Status',
      remainAnnualDays: 'Remaining Annual Leave',
      actions: 'Actions',
    },
    actions: {
      create: 'Create Employee',
      detail: 'Detail',
      edit: 'Edit',
      delete: 'Delete',
    },
    form: {
      createTitle: 'Create Employee',
      editTitle: 'Edit Employee',
      username: 'Username',
      password: 'Password',
      passwordHint: 'Leave blank to keep current password',
      email: 'Email',
      nickname: 'Display Name',
      mobile: 'Mobile',
      sex: 'Gender',
      hiredDate: 'Hire Date',
      status: 'Status',
      remark: 'Remark',
      roles: 'Roles',
      rolesHint: 'Multi-select; permissions of selected roles will be granted to this employee',
      compositeRole: 'Composite role',
      cancel: 'Cancel',
      submit: 'Submit',
    },
    sex: {
      male: 'Male',
      female: 'Female',
    },
    detail: {
      title: 'Employee Detail',
      close: 'Close',
      roles: 'Roles',
      noRoles: 'No roles assigned',
      annualLeave: 'Annual Leave',
      totalAnnualDays: 'Total Days',
      usedAnnualDays: 'Used Days',
      remainAnnualDays: 'Remaining Days',
      otherLeaveDays: 'Other Leave Days',
      notInitialized: 'Not initialized',
      days: 'days',
    },
    delete: {
      title: 'Confirm Delete',
      message: 'Are you sure you want to delete this employee? This action cannot be undone.',
      confirm: 'Delete',
      cancel: 'Cancel',
    },
    validation: {
      usernameRequired: 'Please enter a username',
      nicknameRequired: 'Please enter a display name',
      emailRequired: 'Email is required',
      emailInvalid: 'Please enter a valid email address',
      statusRequired: 'Please select a status',
    },
    error: {
      loadFailed: 'Failed to load employee data',
      saveFailed: 'Failed to save employee',
      deleteFailed: 'Failed to delete employee',
    },
  },
  // File Preview
  filePreview: {
    loadFailed: 'Failed to load file',
    download: 'Download',
  },
  // Regulations
  regulations: {
    title: 'Regulations',
    upload: 'Upload',
    uploadTitle: 'Upload Regulation Document',
    selectFile: 'Select File',
    cancel: 'Cancel',
    table: {
      fileName: 'File Name',
      uploader: 'Uploader',
      uploadDate: 'Upload Date',
      actions: 'Actions',
    },
    noFiles: 'No regulation documents found',
    preview: 'Preview',
    download: 'Download',
  },
}
